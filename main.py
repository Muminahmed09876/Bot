...
async def finalize_post_upload(c: Client, m: Message, uid: int):
    ...
    try:
        # 4. Upload the Photo
        final_file_name = image_path.name
        ...
        if data.get('delete_messages'):
            ...
            try:
                # Delete messages tracking the entire conversation
                await c.delete_messages(chat_id=m.chat.id, message_ids=all_messages_to_delete)
            except Exception as e:
                logger.error(f"Failed to delete post creation messages: {e}")
...
async def cancel_task_cb(c, cb):
    ...
    if uid in TASKS and TASKS[uid]:
        ...
        # New: Clean up post creation state if in progress
        if uid in CREATE_POST_MODE:
            if uid in POST_CREATION_DATA:
                if 'image_path' in POST_CREATION_DATA[uid]:
                    try:
                        Path(POST_CREATION_DATA[uid]['image_path']).unlink(missing_ok=True)
                    except Exception:
                        pass
                POST_CREATION_DATA.pop(uid, None)
            CREATE_POST_MODE.discard(uid)
